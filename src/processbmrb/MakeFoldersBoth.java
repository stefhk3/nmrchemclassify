 ...
 * This only considers structures where hmbc and hsqc exist.
 * Change the classyfire variable to run it for a different classyfire tag.
 *
 */
public class MakeFoldersBoth {
	public static void main(String[] args) throws IOException {
		String classyfire="Superclass";
		File dir=new File("./classyfire");
		File[] files=dir.listFiles();
		File outdirhmbc=new File("./classesboth/"+classyfire+"/hmbc");
		if(!outdirhmbc.exists())
			outdirhmbc.mkdir();
		File outdirhsqc=new File("./classesboth/"+classyfire+"/hsqc");
		if(!outdirhsqc.exists())
			outdirhsqc.mkdir();
		File outdirhmbctrain=new File("./classesboth/"+classyfire+"/hmbc/train");
		if(!outdirhmbctrain.exists())
			outdirhmbctrain.mkdir();
		File outdirhsqctrain=new File("./classesboth/"+classyfire+"/hsqc/train");
		...
		for(File file : files) {
			if(file.getName().endsWith(".sdf")) {
			    ...
			    while (iter.hasNext()) {
			      ...
			      File imagedirhsqc=new File("./classesboth/"+classyfire+"/hsqc/train/"+term);
			      if(!imagedirhsqc.exists())
			    	  imagedirhsqc.mkdirs();
			      File imagedirhmbc=new File("./classesboth/"+classyfire+"/hmbc/train/"+term);
			      ...
			      if(filesimagehmbc.length>0 && filesimagehsqc.length>0) {
			    	  System.out.println(filesimagehmbc[0]);
			    	  File fileto=new File(imagedirhsqc.getPath()+"/"+filesimagehmbc[0].getName());
			    	  if(!fileto.exists())
			    		  Files.copy(filesimagehmbc[0], fileto);			    	  System.out.println(filesimagehsqc[0]);
			    	  fileto=new File(imagedirhmbc.getPath()+"/"+filesimagehsqc[0].getName());
			    	  if(!fileto.exists())
		...
		File outdirhmbctest=new File("./classesboth/"+classyfire+"/hmbc/test");
		if(!outdirhmbctest.exists())
			outdirhmbctest.mkdir();
		File outdirhsqctest=new File("./classesboth/"+classyfire+"/hsqc/test");
		...
		for(File termfile : terms) {
			...
			File outdirhmbctestterm=new File("./classesboth/"+classyfire+"/hmbc/test/"+termfile.getName());
		...
		for(File termfile : terms) {
			File[] images=termfile.listFiles();
			File outdirhsqctestterm=new File("./classesboth/"+classyfire+"/hsqc/test/"+termfile.getName());
