...
public class MakeFolders {
	public static void main(String[] args) throws IOException {
		...
		File outdirhmbc=new File("./classes/"+classyfire+"/hmbc");
		if(!outdirhmbc.exists())
			outdirhmbc.mkdir();
		File outdirhsqc=new File("./classes/"+classyfire+"/hsqc");
		if(!outdirhsqc.exists())
			outdirhsqc.mkdir();
		File outdirhmbctrain=new File("./classes/"+classyfire+"/hmbc/train");
		if(!outdirhmbctrain.exists())
			outdirhmbctrain.mkdir();
		File outdirhsqctrain=new File("./classes/"+classyfire+"/hsqc/train");
		...
		for(File file : files) {
			if(file.getName().endsWith(".sdf")) {
			    ...
			    while (iter.hasNext()) {
			      ...
			      File imagedirhsqc=new File("./classes/"+classyfire+"/hsqc/train/"+term);
			      if(!imagedirhsqc.exists())
			    	  imagedirhsqc.mkdirs();
			      File imagedirhmbc=new File("./classes/"+classyfire+"/hmbc/train/"+term);
			      if(!imagedirhmbc.exists())
			    	  imagedirhmbc.mkdirs();
			      //copy from images hmbc
			      FileFilter fileFilter = new WildcardFileFilter("(2D-HMBC)_"+bmseid+"_nmr_*.png");
			      File imagesDir=new File("./images/");
			      File[] filesimage = imagesDir.listFiles(fileFilter);
			      for(File fileimage : filesimage) {
			    	  System.out.println(fileimage);
			    	  File fileto=new File(imagedirhmbc.getPath()+"/"+fileimage.getName());
			    	  if(!fileto.exists())
			    		  Files.copy(fileimage, fileto);
			      }
			      //copy from images hsqc
			      fileFilter = new WildcardFileFilter("(2D-HSQC)_"+bmseid+"_nmr_*.png");
			      filesimage = imagesDir.listFiles(fileFilter);
			      for(File fileimage : filesimage) {
			    	  System.out.println(fileimage);
			    	  File fileto=new File(imagedirhsqc.getPath()+"/"+fileimage.getName());
			    	  if(!fileto.exists())
			    		  Files.copy(fileimage, fileto);
			      }
			      molcount++;
		...
		File outdirhmbctest=new File("./classes/"+classyfire+"/hmbc/test");
		if(!outdirhmbctest.exists())
			outdirhmbctest.mkdir();
		File outdirhsqctest=new File("./classes/"+classyfire+"/hsqc/test");
		...
		for(File termfile : terms) {
			...
			File outdirhmbctestterm=new File("./classes/"+classyfire+"/hmbc/test/"+termfile.getName());
		...
		for(File termfile : terms) {
			File[] images=termfile.listFiles();
			File outdirhsqctestterm=new File("./classes/"+classyfire+"/hsqc/test/"+termfile.getName());
